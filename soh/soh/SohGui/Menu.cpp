...
void Menu::DrawElement() {
    ...
    if (UIWidgets::Button(ICON_FA_UNDO, options2)) {
        SohGui::mModalWindow->RegisterPopup(
            "Reset Game", "Reset the game now? Any unsaved progress will be lost.", "Reset", "Cancel",
            []() {
...
#ifdef __IOS__
    sidebarWidth = 240 - style.ItemSpacing.x;
#else
    if (menuSize.x > 1600) {
...
#ifdef __IOS__
    if ((sectionWidth - style.ItemSpacing.x * columns) / columns < 720.0f) {
        columns = 1;
    }
#else
    if (windowWidth < 800) {
    ...
    if (headerSearch && menuSearchText.length() > 0) {
        ...
        for (size_t i = 0; i < columnFuncs; i++) {
            ...
            for (auto& entry : sidebar->at(sectionIndex).columnWidgets.at(i)) {
                MenuDrawItem(entry, 90 / columns, menuThemeIndex);
            }
            //}
            if (useColumns) {
                ImGui::EndChild();
